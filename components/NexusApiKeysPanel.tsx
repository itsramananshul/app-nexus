...
interface NexusApiKeysPanelProps {
  open: boolean;
  onClose: () => void;
  nodes: NodeConfig[]; // each node.apiKey here is the env-var-derived key
...
function effectiveKey(
  node: NodeConfig,
  overrides: Record<string, string>,
): { key: string | null; source: "override" | "env" | "none" } {
  const override = overrides[node.id];
  if (override) return { key: override, source: "override" };
  if (node.apiKey) return { key: node.apiKey, source: "env" };
    ...
    <>
      ...
      >
        <header className="flex items-start justify-between border-b border-slate-800 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">
              ...
              and on every collapse mutation. Overrides take precedence over
              the <code className="text-slate-300">NEXT_PUBLIC_*_KEY</code> env
              vars and survive across reloads via localStorage.
            </p>
          </div>
          <button
            type="button"
          ...
          {grouped.map(({ location, members }) => (
            <section key={location} className="mb-5 last:mb-0">
              ...
              <ul className="space-y-2">
                {members.map((node) => {
                  const eff = effectiveKey(node, overrides);
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                          ...
                          {eff.source === "override" ? (
                            <span className="rounded-sm bg-sky-500/15 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-sky-300 ring-1 ring-inset ring-sky-500/30">
                              Override
                            </span>
                          ) : eff.source === "env" ? (
        ...
        <footer className="border-t border-slate-800 bg-slate-950/40 px-5 py-3 text-[11px] text-slate-500">
          Overrides live in your browser&apos;s localStorage. Clear an
          override to fall back to the <code className="text-slate-300">
            NEXT_PUBLIC_*_KEY
          </code>{" "}
          env var for that node.
        </footer>
      </aside>
    </>
  );
